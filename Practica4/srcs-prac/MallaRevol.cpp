...
MallaRevol::MallaRevol( const char * nombre_arch, unsigned nperfiles, bool coordtext, bool esTapa){
    ...
    if(coordtext){
        float s;
        float d[long_perfil];
        coords_textura.resize(long_perfil*nperfiles);
        d[0] = 0;
        
        for(int i = 1 ; i < long_perfil ; i++ ){
            d[i] = d[i-1]+norma(tabla_vertices[i]-tabla_vertices[i-1]);
        }
        
        for (int i = 0 ; i < nperfiles ; i++){
            s = (float)i / (nperfiles-1);
            for(int j = 0 ; j < long_perfil ; j++ )
                coords_textura[i*long_perfil+j] = Tupla2f(s,d[j]/d[long_perfil-1]);
