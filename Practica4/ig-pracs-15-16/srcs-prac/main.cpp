...
void FGE_PulsarTeclaNormal( unsigned char tecla, int x_raton, int y_raton )
{
   ...
   switch (toupper(tecla))
   {
      ...
      case 'M' :
		if (modo_vis == 0){
			modo_vis = 1;
			std::cout << "\nCambiado al modo alambre.\n";
		}
		else if (modo_vis == 1){
			modo_vis = 2;
			std::cout << "\nCambiado al modo sólido.\n";
		}
		else if (modo_vis == 2){
			modo_vis = 3;
			std::cout << "\nCambiado al modo ajedrez.\n";
		}
		else if (modo_vis == 3){
			modo_vis = 0;
			std::cout << "\nCambiado al modo puntos.\n";
		}
		break;
	  case 'P' :
...
/*
// Función que se llama para modificar los parámetros.
// Esta función de animación debe cambiar los parámetros del modelo en función de la velocidad y para que se redibuje, llama a glutPostRedisplay();
void idle(void){  
	
	//P3_FGE_PulsarTeclaNormal( tecla );
	//  modifique variables 
...
int main( int argc, char *argv[] )
{
   ...
   /* Función de fondo que se utiliza para animar el modelo. Se ejecuta de forma indefinida en el ciclo de ejecución
    * de la aplicación OpenGL, en la que se puede realizar la actualización de cada parámetro en función de su velocidad.
    * Se crea en el fichero principal y se activa desde el programa principal con:
    */
   //glutIdleFunc(idle);
