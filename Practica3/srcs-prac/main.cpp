...
const int numero_modos = 4; //(0,1,3,4)
const int numero_practicas = 3; // En la practica 3 hay tres practicas (la 1, la 2 y la 3)

int
modo_vis  ,  // modo de visualización (0,1,3,4)
practica_activa;  // practica actual (cambiable por teclado) (P)
...
void FGE_PulsarTeclaNormal( unsigned char tecla, int x_raton, int y_raton )
{
    ...
    switch (toupper(tecla))
    {
        ...
        case '-' :
            ...
            break;
        case 'M':
            modo_vis = (modo_vis+1)%numero_modos;
            redibujar = true;
            break;
            //Cambiar de practica
        case 'P':
            practica_activa++;
            if (practica_activa == numero_practicas+1) {
                practica_activa = 1;
            }
            redibujar = true;
            std::cout << practica_activa << endl;
            break;
        default:
            redibujar = false ;
            switch( practica_activa )
